function [G_opt, mu_opt, mu_opt_div_G, kfit_final] = global_fit_data_to_curve(rho, rho_l, c_l, cp, h, G0, mu0, mu_factor, f,k,VdB, f_reduced_idx)
    if nargin < 1
    ...
    ub = [1e6]; % No upper bound in this case
    
    gs = GlobalSearch('NumTrialPoints', 10, 'NumStageOnePoints', 3, 'Display', 'iter');
    ...
    objfun,'x0',20e3,'lb',lb,'ub',1e6,'options',opts);
    

    [popt, fval_opt] = run(gs, problem);
