...
mu0 = 150E3;    % Initial guess for mu (Pa)
...
%hold off
%VdB = y_est';
% Set bounds for G and mu
lb = [0, 0];       % Lower bounds
ub = [1e6, 1e6];   % Upper bounds (G <= 10^7, mu <= 10^7)
...
[popt, fval_opt] = fmincon(objfun, [G0, mu0], [], [], [], [], lb, ub, [], options);
...
plot(f, kfit, 'r-', 'LineWidth', 2);

plot(f(f_reduced), VdB, 'b-o', 'LineWidth', 2);
plot(f, kfit_0, 'g--', 'LineWidth', 2)
legend('Simulated Fit', 'Data Points', 'Original Guess')
